...
- AP思想：各个子事务分别执行和提交，无需锁定数据。允许出现结果不一致，然后采用弥补措施恢复，实现最终一致即可。例如`AT`1模式就是如此
...
> * 但在senta内部重试是有一个限制的，重试最多30次，但等待DB锁的过程是比较长的，所以事务2会先等不及，就会回滚业务，释放DB锁，所以并没有成功把money设置为80
