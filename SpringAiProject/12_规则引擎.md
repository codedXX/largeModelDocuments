...
2、在**不重启服务的情况下可随时对业务规则进行扩展和维护** ⭐️
...
> 
>
> 这里是 **DRL 规则文件的包与类型导入声明**，作用和 Java 文件中的 `package`、`import` 很相似。
>
> ```
> package com.order
> import com.drools.model.Order
> ```
>
> `package com.order` 表示这组规则属于 `com.order` 规则命名空间。它主要用于组织和区分规则，例如以后可以有：
>
> ```
> package com.order      // 订单规则
> package com.user       // 用户等级规则
> package com.coupon     // 优惠券规则
> ```
>
> 它不要求必须存在对应的 Java 包或目录；当前的 `com.order` 只是规则包名。实际项目一般让规则包名与业务含义或目录结构保持一致，便于维护。
>
> `import com.drools.model.Order` 则是导入 Java 的事实类。这样规则中可以简写：
>
> ```
> $order : Order(amout >= 1000)
> ```
...
>
> ```
> $order : com.drools.model.Order(amout >= 1000)
> ```
>
> 所以这两行分别解决两件事：
>
> - `package`：这条规则“属于哪里”。
> - `import`：规则要匹配的 `Order` 类型“来自哪里”。
...
> **session.insert(order); 这里固定是insert方法吗?**
>
> 
>
> 是的，向 Drools 的工作内存加入事实对象时，通常使用固定的 API：
>
> ```java
> FactHandle handle = session.insert(order);
> ```
>
> `session.insert(order)` 就是把 `Order` 作为 Fact 插入当前 `KieSession`。返回的 `FactHandle` 是该对象在规则引擎内的引用；当前示例不需要保存它，所以可以省略返回值：
>
> ```
> session.insert(order);
> session.fireAllRules();
> ```
>
> 如果插入后修改了对象，并希望 Drools 重新判断规则，应使用：
>
> ```
> order.setAmout(200);
...
> ```
>
> 这里的 `insert` 不是 Java 关键字，而是 `KieSession` 接口定义的方法。



> ⭐️**为什么写法这么奇怪？
> 有时候 $order:Order(amout < 100)这样写，会出现一个Order,
> 有时候 $order.setScore(0); 这样写，又不出现Order。**
>
> 
>
> 这是因为两处处在规则的不同区域，职责不一样。
>
> ```
> when
>     $order : Order(amout < 100)
...
> - `Order` 表示要找的 Java 类型；
> - `amout < 100` 是筛选条件；
> - $order 给找到的那个 `Order` 对象起一个变量名。
>
> 它可以理解为：
>
> ```
> Order order = 工作内存中金额小于100的订单;
> ```
>
> 而在：
>
...
> ```
>
> 这里已经通过前面的匹配拿到了对象，并把它保存到 $order 变量里，因此只需用变量调用方法，不需要再写类型 `Order`。
>
> 类似 Java：
>
> ```
> Order order = new Order(); // 声明类型时写 Order
> order.setScore(0);         // 已有变量后只写 order
> ```
>
> 整条规则可以按下面理解：
>
> ```
> $order : Order(amout < 100)  // 找到一个 Order，并命名为 $order
> $order.setScore(0);          // 使用刚才找到的对象
> ```
