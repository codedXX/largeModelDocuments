...
> ⭐⭐⭐那么通常情况下，我们查询的时候肯定优先去查Redis，但如果你是第一次查，那肯定Redis是无法命中的，那么这个时候呢，你就要去查数据库了。数据库只要你请求参数是没问题的，那么数据库应该是能查到的。那查到了以后呢，我们就可以把数据库里得到的数据给它缓存到Redis当中，然后再把查询到的结果返回给客户。那么这样一来，Redis里缓存了结果，以后你再来查的时候，是不是肯定就可以命中并把直接返回，这样是不是就不用查数据库了？所以这就是缓存的一个作用。
>
> 那需要注意的是，我们在把数据库中的数据啊，缓存到Redis的时候，建议大家都给它设置一个TTL，过期时间，因为这样的话呢，假如说啊，将来数据库操作完，我们去更新Redis的时候呢，更新失败了也不用担心，因为这个缓存是有过期时间的嘛，到期了以后呢，它也会从Redis里给移除，那这样呢，下次再来查的时候肯定会命中呢，肯定就要去查数据库里边最新的数据，并且重新写入到Redis，相当于是把数据库最新的数据去做重建缓存。那这样一来是不是也就相当于实现了数据库与缓存的这个一致性了。所以呢，我们给缓存加过期时间，相当于是给你的这种一致性加了一个兜底的方案。也就是说在正常去更新缓存失败的情况下，也可以利用过期清理这种方案去保证它。数据库的一致。
>
> 这是Cache Aside这种模式下，查询的一个流程。那接着我们再来看它的增删改的流程。那按照CacheAside的说明，我们需要在更新数据库的同时去更新缓存。因此呢，当有任何的增删改的业务发生时，你一定也要在Redis里去执行增删改，对吧？当然同时呢，我们要在数据库里完成增删改的操作。那么当两者都完成之后呢，再去返回结果给用户。并且啊，在这个过程当中呢，我们还要去确保Redis的操作与数据库的操作得是同时成功、同时失败的，确保一致性。这是Cache Aside才增删改的基本流程
>
> ![](../SpringCloudImages/36.png)
...
> * ⭐**第一个细节：**
>
>   * 在去做数据库增删改的同时Redis的要求也得改。也就是数据库要增,你也要增,数据库删,你这边也要删。数据库要改,你这边也要改。但是是不是真的有必要大家得做一模一样的操作呢?我们仔细思考一下啊。我们举个例子,比如说现在,我要向数据库里新增一条数据。Redis里是不是也要说我要立刻也去新增一条呢?有没有必要?我觉得是没必要。你想嘛,我现在数据库里新增一条,你立马往Redis里写了一条。我又增一个,你也增一个。假如说你增了这条数据,一直以来都没有人看,怎么办呢?你往Redis写是不是一种浪费啊?对不对?你占用内存啊。那你说那我不写怎么办?不写也没有关系。数据库里咔咔一顿写，现Redis是不是没有?没有,假如说有一天有人来查了,那他一看Redis里没有,他就会去查数据库。数据库一查,是不是就写到Redis里去了，那这样缓存不就建立了吗。所以说,当数据库新增的时候,Redis根本没必要新增,纯粹是在浪费。当谁来查的时候,我再去建立缓存,不就完了吗?你永远不查,我就永远不写。对不对?**所以这就是第一个,就是增的时候,显然Redis是不用去做增的。**
>   * 那删的时候呢?我们把数据库里某一条数据删了,Redis要不要跟着删啊?这个显然是要删的,对不对?因为你如果不删的话呢,等于这条数据在数据库里没了,Redis里还有,人家查的时候,结果一看,哎,这个数据还在,他以为这条数据没删。这就问题了。**所以删是要做的**。
>   * 那么改呢?我现在对数据库里的某条数据做了一个修改,那Redis要不要立刻修改啊?这时候你可以改啊,但是,我的建议是不要做改,而是直接做删除操作，为什么呢?其实跟新增的道理是一样的。如果我现在的去修改了数据库里的某一条数据了,然后你的Redis里也跟着去修改了。但是呢,现在如果根本就没有人查,那你的这一次Redis的操作是不是浪费的?啊,然后我又去修改Redis里的数据库里的某一条数据。频繁的去修改,你就频繁的去改Redis。但是从始至终就一直没有人去查Redis。那你这个操作的意义是什么?对不对?所以,我建议你做删除的原因是什么呢?每一次我来更新数据库里面的数据的话,我Redis就删掉就完了。删掉以后,那Redis里是不是就没有缓存了?没有缓存没关系,啊,对不对?什么时候你来查了,我再去建立缓存。这样是不是也可以减少不必要的Redis的操作的内存浪费啊?因此呢,综上所述,当我们对数据库去做增删改操作的时候,其实Redis,**没有必要同步的增删改，而是只要去做删除就行了**。这样可以尽可能的减少对Redis不必要的内存占用啊,同时也不影响整个缓存的效率。除非你现在这个数据它是一个非常热点的数据,就是说它的访问频率特别特别高的,你需要它每时每刻都有缓存。那这时候呢,你可以在每次增删改的时候,你也同步去修改Redis,这没问题。但是大多数的数据来讲,我们认为就没有必要。这就是CacheAside模式在企业里面的最佳实践方案。**数据库增删改,Redis直接做删除就行了**。**这是第一个细节**。![](../SpringCloudImages/38.png)
>
> * ⭐⭐**第二个细节：**
>
>   * 第二个细节就是两个操作的一个顺序问题。我们到底是**先操作Redis还是先操作数据库**?这里面也是有讲究的。那我们先来看目前呢能看到的这种方案,就是先删Redis再去做数据库的操作,这种方案它有没有什么问题?这里面呢其实会有一个线程的安全问题,我给大家分析一下。现在假如说我们**并发的有两个线程**,这两个线程刚好一个来去查数据,一个来去修改数据。我们假如说操作的是同一条数据。假如说修改数据这个线程先来。那么当这个线程到达了以后呢,它这个请求就来了,我们是不是要优先去先把Redis给删掉?那么这个就先执行了，现在Redis里就没了，没了以后呢,接着我是不是要去操作数据库了?对吧?我把数据库里的这条数据给改了。那这样一来你下次再查的时候是不是就能发现未命中，然后你再去查最新的就行了。这理论上是这这样的。但是现在是多线程并发运行。那么在我们删除完Redis还没来得及去更新数据库的时候,就在此时一个查询的请求来了,那它会干什么?它会去Redis里查，它一查发现什么了?当然由于你删了,这Redis里没了呀，Redis没了它会干什么?它会去查数据库。它会立刻去查数据库。那问大家,查数据库快还是改数据库块?显然是查得快。所以说呢,有可能这个查的命令就先执行了。由于更新数据库的操作还没做完，但它先查了，它查到的就是一个旧的数据，然后会把旧的数据缓存到redis，然后刚刚删除的操作就是白删了,redis建立了一个新的缓存，然后里面的数据还是旧的，然后这时候刚刚第一线程缓过劲儿来了，去写数据库了，这时候数据库就变成新的了，而缓存里还是旧数据，两者出现了不一致，这就是**并发安全问题**，这种问题出现概率比较高。所以**建议先操作数据，再删redis**。![](../SpringCloudImages/39.png)
>
>     ![](../SpringCloudImages/40.png)
>
>   * 上述建议的【**建议先操作数据，再删redis**】方案也**并非完美无缺**，也有一定概率发生线程安全问题，就是下方列出的**第二个【异常情况】**
...
可以发现，异常状态发生的概率极为苛刻，线程1必须是查询数据库已经完成，但是缓存尚未写入之前。线程2要完成更新数据库同时删除缓存的两个操作。要知道线程1执行写缓存的速度在毫秒之间，速度非常快，在这么短的时间要完成数据库和缓存的操作，**概率非常之低。**而且就算发生了，**我们还设置了缓存数据的TTL，也能够保证redis的同步**
