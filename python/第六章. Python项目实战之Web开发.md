...
# 挂载静态文件：浏览器访问 /static/文件名时，从本地 static 目录返回对应文件；
# name="static" 是该挂载服务的内部路由名称，供 url_for("static", path="文件名") 生成访问地址。
...
client = OpenAI(api_key="sk-d6809f8523c840bc9f1735c544080d5a", base_url="https://api.deepseek.com")
...
def create_session() -> ApiResponse:
    ...
    with open(f"sessions/{session_id}.json", "w", encoding="utf-8") as f:
        # ensure_ascii=False：保存中文时保持中文，不转换成 Unicode 编码。
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="localhost", port=8000) # access_log=False : 关闭访问日志
