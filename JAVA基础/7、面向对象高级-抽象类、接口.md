...
- 用 `enum` 代替 `class`。
- 里面列出几个**常量**（习惯全大写），用逗号分隔，结尾分号可省（无其它内容时）。
- 这些常量就是这个枚举仅有的几个实例。
...
// 常配合 switch
switch (s) {
    case NORMAL -> System.out.println("正常");
    case FROZEN -> System.out.println("冻结");
}
```

#### 二、带属性的枚举（最常用 ⭐）

实际开发中，枚举常常要**关联一些值**（比如状态码、描述）。这时给枚举加**字段 + 构造方法**：
...
- `NORMAL(1, "正常")` 里的参数会传给构造方法。
- 构造方法**默认私有**，不能 `new Status()`。
- 字段一般用 `final`，配 getter 对外暴露。
...
System.out.println(s.getDesc());  // 正常
```
...
> ⭐⭐**NORMAL(1, "正常")会做什么操作**
>
> * `NORMAL(1, "正常")` 这一行，本质上就是在**创建一个 Status 枚举的实例**——它会**调用构造方法**。
>
> * 你可以把它**等价理解**成（虽然语法上不能这么写）：
>
>   ~~~java
>   public static final Status NORMAL = new Status(1, "正常");
>   ~~~
>
> * 也就是说，`NORMAL(1, "正常")` 干了三件事：
>
>   1. **`new` 一个 Status 对象**
>   2. 把括号里的 `1` 和 `"正常"` 作为参数，**传给构造方法**
>   3. 把这个对象赋值给一个名为 `NORMAL` 的 `public static final` 常量



#### 三、加个工具方法（按 code 反查枚举）

经常需要"根据数据库存的 code（1/2）找到对应枚举"，可以加静态方法：
...
public enum Status {
    ...
    public static Status of(Integer code) {
        for (Status s : values()) {   // values() 返回所有枚举值
...
```
Status.values()        // 所有枚举值数组：[NORMAL, FROZEN]
Status.valueOf("NORMAL") // 按名字得到枚举：NORMAL
s.name()               // 枚举的名字字符串："NORMAL"
s.ordinal()            // 序号（从0开始）：NORMAL=0, FROZEN=1
```

> ⚠️ 别用 `ordinal()` 当业务编号存数据库——一旦调整枚举顺序，序号就变了，会出事故。要存编号就自己定义 `code` 字段（像上面那样）。

#### 五、用 Lombok 简化（如果项目用了 Lombok）

字段的 getter 可以用 `@Getter` 省掉：
