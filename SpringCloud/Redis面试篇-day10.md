# Redis面试篇（day10）：主从 · 哨兵 · 分片集群 · 数据结构 · 缓存问题
...
> ⭐⭐⭐**services代表就是一个工程, 里面包含了多个服务**
>
> - 其中第一个服务叫R1, 第二个叫R2, 第三个叫R3, 也就是我们的三个redis实例, 那他们采用的镜像呢, 都是redis, 他们的容器名分别叫R1, R2和R3
> - ⭐⭐⭐他们的**网络模式都叫host**, 这个地方要给大家解释一下。我们之前在去创建容器的时候, 从来没有配置过网络模式, 那么这种情况下的网络模式呢, 都是默认的桥接, 那么桥接模式呢, 其实就是由docker给你提供的一个虚拟的网桥, 基于它来去与宿主机建立一个连接, 那这个时候呢, 每一个容器都有自己这个虚拟的IP地址, 对吧? 那么这样一来呢, 我们的各个redis实例之间要进行一个交互啊, 就需要先通过虚拟网桥, 再到宿主机网卡这样的一个转换。网络的一个转发。那么这个时候呢, 集群之间的这种健康检测呢, 就可能会出现一定的问题。那么在redis官方, 它就告诉我们了, 你再去做搭建这种集群时, 请不要使用桥接模式, 尽量使用那个host模式。那么host模式怎么回事呢? 它是没有借助于虚拟网桥, 而是让我们的每个容器直接使用宿主机的网络。那这样一来, 这一些容器相当于直接暴露在宿主机的网络里面, 相当于宿主机的一个普通进程了。因此呢, 你将来去部署的时候, 你也不用去做端口映射了, 而是直接就是使用宿主机的端口, 就行了。啊, 所以这是大家需要注意的, 将来我们在去搭建redis集群的时候, 不管是redis任意一种集群, **你的网络模式, 一定要采用这种host模式。**



将其上传至虚拟机的`/root/redis`目录下：

![](../SpringCloudImages/f10-003.png)
...
![](../SpringCloudImages/f10-004.png)

查看docker容器，发现都正常启动了：

![](../SpringCloudImages/f10-005.png)

由于采用的是host模式，我们看不到端口映射。不过能直接在宿主机通过ps命令查看到Redis进程：

![](../SpringCloudImages/f10-006.png)
...
那么这个同步是如何完成的呢？
![](../SpringCloudImages/30.png)

![](../SpringCloudImages/31.png)

#### 1.3.1.全量同步

主从第一次建立连接时，会执行**全量同步**，将master节点的所有数据都拷贝给slave节点，流程：

![](../SpringCloudImages/f10-007.png)

**这里有一个问题，`master`如何得知`salve`是否是第一次来同步呢？？**

> 连接上redis后，例如执行docker exec -it r2 redis-cli -p 7002 后 可以通过命令**info replication 去查看master_replid**
...
![](../SpringCloudImages/f10-009.png)

再看下`r2`节点执行`replicaof`命令时的日志：

![](../SpringCloudImages/f10-010.png)
...
![](../SpringCloudImages/f10-012.png)

slave与master的offset之间的差异，就是salve需要增量拷贝的数据了。

随着不断有数据写入，master的offset逐渐变大，slave也不断的拷贝，追赶master的offset：

![](../SpringCloudImages/f10-013.png)

直到数组被填满：

![](../SpringCloudImages/f10-014.png)

此时，如果有新的数据写入，就会覆盖数组中的旧数据。不过，旧的数据只要是绿色的，说明是已经被同步到slave的数据，即便被覆盖了也没什么影响。因为未同步的仅仅是红色部分：

![](../SpringCloudImages/f10-015.png)

但是，如果slave出现网络阻塞，导致master的`offset`远远超过了slave的`offset`：

![](../SpringCloudImages/f10-016.png)

如果master继续写入新数据，master的`offset`就会覆盖`repl_baklog`中旧的数据，直到将slave现在的`offset`也覆盖：

![](../SpringCloudImages/f10-017.png)
...
- **在master中配置`repl-diskless-sync  yes`启用无磁盘复制，避免全量同步时的磁盘IO。**
  - 因为正常情况下，同步的时候需要把所有数据写到磁盘，再通过网络传给从节点，传给从节点的时候还要从磁盘度过来，所以有大量的磁盘IO
  - ⭐**repl-diskless-sync就是无磁盘复制，意思就是主节点直接通过网络传给从节点，就可以大量减少磁盘IO**

- Redis单节点上的内存占用不要太大，减少RDB导致的过多磁盘IO
  - **可以多建立几个主从集群**，不同的业务用不同的redis，做一个分离
    *  **一般不要超过8G**

- 适当提高`repl_baklog`的大小，发现slave宕机时尽快实现故障恢复，尽可能避免全量同步
- 限制一个master上的slave节点数量，如果实在是太多slave，则可以采用`主-从-从`链式结构，减少master压力
  - ⭐**`主-从-从`的意思是：让一部分从节点以其他的从节点作为主节点，形成多级主从的关系**
...
`Sentinel`基于心跳机制监测服务状态，**每隔1秒**向集群的每个节点发送ping命令，并通过实例的响应结果来做出判断：

- **主观下线（sdown）**：如果某sentinel节点发现某Redis节点未在规定时间响应，则认为该节点主观下线。
  - 这个master不一定真的就是宕机了，有可能是网络故障，也有可能是其他人网络故障，反正就是响应阻塞了，或者仅仅是超时了，可能过一会儿响应就成功了，这种情况就叫做**主管下线**，就是**个人认为下线了**。

- **客观下线(odown)**：若超过指定数量（通过`quorum`设置）的sentinel都认为该节点主观下线，则该节点**客观下线**。quorum值最好超过Sentinel节点数量的一半，Sentinel节点数量至少3台。



如图：

![](../SpringCloudImages/f10-020.png)
...
对应的官方文档如下：https://redis.io/docs/latest/management/sentinel/#replica-selection-and-priority
...
> * 这里的hmaster是要监听的那个主从集群的名称，这个名称随便定义
>
> * hmaster后面跟的192.168.150.101 是要监控的主从集群中的主节点的IP
>   * 因为知道主节点的IP，自然就知道所有从节点的IP
> * 192.168.150.101 后面的 2   就是quorum值
...
- `sentinel down-after-milliseconds hmaster 5000`：声明master节点超时多久后被标记下线
  - 就是ping hmaster节点的时候，超过5000ms没响应，就认定为超时
...
> * ⭐**用"redis-server"启动的就是普通的redis节点，用来做普通的增删改查都可以**
> * ⭐**用"redis-sentinel启动的就是哨兵节点，用来监控**
...
- 海量数据存储
  - ⭐**主从集群里不管集群中有多少个节点，但主和从之间都是数据备份的关系，也就是说他们数据都是一样的，因此他们的容量上限就是单节点redis容量的上线**
    -   **单节点redis容量上限一般不要超过8GB**
...
要解决这两个问题就需要用到**分片集群**了。分片的意思，就是把数据拆分存储到不同节点，这样整个集群的存储数据量就更大了。

Redis分片集群的结构如图：

![](../SpringCloudImages/f10-029.jpg)
...
https://security.feishu.cn/link/safety?target=https%3A%2F%2Fredis.io%2Fdocs%2Fmanagement%2Fscaling%2F%23redis-cluster-101&scene=ccm&logParams=%7B%22location%22%3A%22ccm_docs%22%7D&lang=zh-CN
...
![](../SpringCloudImages/f10-038.png)

过了一段时间后，7002原本的小弟7006变成了`master`：

![](../SpringCloudImages/f10-039.png)
...
![](../SpringCloudImages/f10-041.png)

我们可以看到1号元素就有指向3、5、10的多个指针，查询时就可以跳跃查找。例如我们要找大小为14的元素，**查找的流程是这样的**：

> ⭐**下面的图就是分了四级，从下到上分别是：一级指针 -> 二级指针 -> 三级指针 -> 四级指针**

![](../SpringCloudImages/f10-042.png)
...
![](../SpringCloudImages/f10-044.png)



## 5.Redis内存回收

Redis之所以性能强，最主要的原因就是**基于内存存储**。然而单节点的Redis其内存大小不宜过大，会影响持久化或主从同步性能。
...
对于某些特别依赖于Redis的项目而言，仅仅依靠过期KEY清理是不够的，内存可能很快就达到上限。因此Redis允许设置内存告警阈值，当内存使用达到阈值时就会主动挑选**部分KEY**删除以释放更多内存。这叫做**内存淘汰**机制。
...
![](../SpringCloudImages/f10-047.png)

**异常情况**

![](../SpringCloudImages/f10-048.png)
...
![](../SpringCloudImages/f10-049.png)

**异常情况**

![](../SpringCloudImages/f10-050.png)
...
![](../SpringCloudImages/f10-054.png)

### 6.3.缓存雪崩

缓存雪崩是指在同一时段大量的缓存key同时失效或者Redis服务宕机，导致大量请求到达数据库，带来巨大压力。

![](../SpringCloudImages/f10-055.png)
...
![](../SpringCloudImages/f10-057.png)

逻辑过期的思路如图：

![](../SpringCloudImages/f10-058.png)
