# MQ高级（day07）：发送者可靠性 · MQ可靠性 · 消费者可靠性 · 延迟消息
...
因此，这里我们必须尽可能确保 MQ消息的可靠性，即：消息应该至少被消费者处理1次
...
> **注意：** 
>
> * 重试等待间隔是1S，但实际发现是2S，为什么呢？
>   * 因为有个connection-timeout连接超时时间为1s，所以合起来就是**建立连接，然后等了1S发现失败，失败后又等1S再发第二次，合起来就是2S**
...
![](../SpringCloudImages/f07-001.png)

**⭐返回的结果有以下几种情况：**

- 当消息投递到MQ，但是路由失败时，通过**Publisher Return**返回异常信息，同时返回**ACK**的确认信息，代表投递成功
  - 这种情况就是代码写的有问题

- **临时消息**投递到了MQ，并且入队成功，返回**ACK**，告知投递成功
  - ⭐⭐**什么是临时消息呢？**
    - **以前讲过一个概念叫持久化，持久化就是把一个数据写到磁盘，确保它的安全，以后就算服务宕机了也不会丢失，临时消息就是反之，就不是持久的。**

- **持久消息**投递到了MQ，并且入队完成持久化，返回**ACK** ，告知投递成功
  - 持久消息就是消息写到了磁盘
  - **对于持久化消息来说，必须写到了磁盘才算成功，没写到磁盘都算失败**
...
![](../SpringCloudImages/f07-012.png)

放行以后，由于抛出的是**消息转换异常**，因此Spring会自动返回`reject`，所以消息依然会被删除：

![](../SpringCloudImages/f07-013.png)
...
![](../SpringCloudImages/f07-022.png)

当然，也可以直接使用课前资料提供好的插件：

![](../SpringCloudImages/f07-023.png)
