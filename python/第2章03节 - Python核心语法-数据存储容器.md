...
> **注意：** 字典（`dict`）中的 `value` 可以是`任何类型的数据`，而 `key` 不能为可变类型（如：不能为列表 `list`、集合 `set`、字典 `dict`）。
