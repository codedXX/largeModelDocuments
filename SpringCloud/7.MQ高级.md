...
> 类似这样的日志
>
> * `nio-8081-exec-*`：生产者请求，最多尝试 3 次
>
> ~~~
> 2026-08-14 17:19:04  INFO 9500 --- [tContainer#0-36] o.s.a.r.c.CachingConnectionFactory       : Attempting to connect to: [localhost:5672]
> 2026-08-14 17:19:04  INFO 9500 --- [nio-8081-exec-8] o.s.a.r.c.CachingConnectionFactory       : Attempting to connect to: [localhost:5672]
> ~~~
...
> ⭐⭐**生产环境建议同时开启。两者负责的阶段不同**
>
> * `ConfirmCallback：消息是否到达交换机`
> * `ReturnsCallback：消息是否从交换机路由到队列`
...
> 
>
> `acknowledge-mode: auto` 表示由 Spring 的监听容器替你调用 ACK/NACK，而不是 RabbitMQ 一投递就自动确认。
>
> 流程是：
>
> ```
...
我们在用户支付成功后会发送MQ消息到交易服务，修改订单状态为已支付，就可能出现消息重复投递的情况。如果`消费者不做判断，很有可能导致消息被消费多次，出现业务故障`。
...
> ⭐️RabbitMQ为什么会存在**业务幂等性这个说法**?
>
> 
>
> RabbitMQ 通常提供的是“至少一次投递”语义，不保证业务操作只执行一次。
>
> 
>
> 例如消费者已经把订单状态更新、扣库存或写入数据库，但还没来得及 `ack` 就崩溃了。RabbitMQ 会认为消息没处理成功，随后重新投递；消费者重试、网络超时、生产端重发也都可能造成重复消息。
>
> 
>
> RabbitMQ 无法判断“这条消息对应的订单是否已经扣过库存”，因此`需要业务自己保证：同一业务消息被处理多次，最终结果和处理一次相同，这就是业务幂等性。`
>
> 
>
> **常见做法：**
>
> - 消息携带唯一业务 ID，例如 `orderId`、`eventId`
> - 数据库加唯一索引或记录消费状态
> - 用条件更新，例如仅当订单仍是 `PENDING` 时才改为 `PAID`
> - 扣减、转账等关键动作用幂等键或去重表保护
>
> 
>
> 所以，“业务幂等性”不是 RabbitMQ 的额外功能，而是应用为适配消息重复投递、重试和故障恢复所必须承担的语义。
