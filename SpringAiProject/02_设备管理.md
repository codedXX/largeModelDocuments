...
| ----------- | ------------------------------------------------------------ |
| `@Null`     | 验证对象必须为 `null`。                                      |
| `@NotNull`  | 验证对象不能为 `null`；字符串可以是空串 `""`。               |
| `@NotBlank` | 仅用于字符串。要求不为 `null`，且去除首尾空白后长度大于 0。  |
| `@NotEmpty` | 验证对象不能为 `null` 或空：字符串不能为空串、集合不能为空集合、数组不能为空数组、Map 不能为空。 |

**🔹Boolean 检查**

| 注解           | 说明                              |
| -------------- | --------------------------------- |
| `@AssertTrue`  | 验证 `Boolean` 值必须为 `true`。  |
| `@AssertFalse` | 验证 `Boolean` 值必须为 `false`。 |

**🔹长度检查**

| 注解                      | 说明                                                         |
| ------------------------- | ------------------------------------------------------------ |
| `@Size(min = , max = )`   | 验证 `Array`、`Collection`、`Map`、`String` 的长度或大小是否在指定范围内。 |
| `@Length(min = , max = )` | 验证字符串长度是否在指定范围内。Hibernate Validator 扩展注解。 |
...
| ------------------ | ------------------------------------ |
| `@Past`            | 验证日期时间必须早于当前时间。       |
| `@Future`          | 验证日期时间必须晚于当前时间。       |
| `@PastOrPresent`   | 验证日期时间必须早于或等于当前时间。 |
| `@FutureOrPresent` | 验证日期时间必须晚于或等于当前时间。 |

> `@Pattern` 属于字符串格式检查，不属于日期专用注解。
...
| `@Email`                | 验证是否为合法邮箱格式；值为 `null` 时默认校验通过，需配合 `@NotNull` 使用。 |
| `@URL`                  | 验证是否为合法 URL，可限制协议、主机、端口及正则规则。       |

**🔹数值检查**

> 建议用于 `String`、`Integer`、`BigDecimal` 等包装类型；不建议直接用于基本类型 `int`。表单空值 `""` 无法转换为 `int`，但可转换为 `String` 的空串或包装类型的 `null`。

| 注解                               | 说明                                                     |
| ---------------------------------- | -------------------------------------------------------- |
| `@Min(value)`                      | 验证数值不小于指定值。                                   |
| `@Max(value)`                      | 验证数值不大于指定值。                                   |
| `@DecimalMin("value")`             | 验证数值不小于指定的小数值，适合精确小数比较。           |
| `@DecimalMax("value")`             | 验证数值不大于指定的小数值，适合精确小数比较。           |
| `@Digits(integer = , fraction = )` | 验证数值的整数位数与小数位数是否符合要求。               |
| `@Range(min = , max = )`           | 验证值是否处于指定范围内。Hibernate Validator 扩展注解。 |

示例：

```
@Range(min = 10000, max = 50000, message = "工资必须在 10000 到 50000 之间")
...
| 注解     | 说明                                                         |
| -------- | ------------------------------------------------------------ |
| `@Valid` | 用于方法参数或对象属性，递归校验关联对象。关联对象为集合、数组时，会校验其中每个元素；为 `Map` 时，会校验其值。 |
...
| 注解                                                | 说明                             |
| --------------------------------------------------- | -------------------------------- |
| `@CreditCardNumber`                                 | 验证信用卡号格式。               |
| `@ScriptAssert(lang = "", script = "", alias = "")` | 使用脚本表达式进行对象级别校验。 |
...
![img](../SpringAiProjectImages/788db5ab-31c3-4fa4-bf9c-881c3d09ec54.png)
