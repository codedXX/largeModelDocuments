# Elasticsearch02（day09）：DSL查询 · RestClient查询 · 数据聚合
...
![](../SpringCloudImages/f09-001.png)

你会发现虽然是**match_all，但是响应结果中并不会包含索引库中的所有文档，而是仅有10条。**这是因为处于安全考虑，elasticsearch设置了默认的查询页数。
...
- **精确查询（Term-level queries）**：不对用户输入搜索条件分词，根据字段内容精确值匹配。但只能查找keyword、数值、日期、boolean类型的字段。例如：
  ...
  > ⭐⭐⭐**对于精确查询一般输入的都是比较简单的且不可分割的词，就用户输入的就是一些不可分割的词条或者词语或者值，比如说人的名字、数值、日期、boolean**
...
> ⭐全文检索查询时因为用户输入了搜索关键字，所以带来一个问题，哪些文档跟用户输入的内容关联度更高、匹配度更高，全文检索查询默认会按照匹配度去做一个排名，排名越高的越往前。
>
> * **怎么知道哪个排名更高、更往前？**
>   * 因为它内部有一个打分机制，会给每一个文档与词条的匹配度进行打分，具体分数就是**_score这个字段**
...
![](../SpringCloudImages/f09-005.png)

当你输入的搜索条件不是词条，而是短语时，由于不做分词，你反而搜索不到：

![](../SpringCloudImages/f09-006.png)



> 需要注意的是：在下述图片中，**为什么搜索“脱脂牛奶”的结果是空的？**
>
> * ⭐⭐**因为在精确查询里“脱脂牛奶”是不会分词的，但是ES会对查询数据里面的name属性分词，分词之后的词条有“脱脂”和“牛奶”，但没有“脱脂牛奶”这个词条，所以查询结果就是空的**
>
> ![](../SpringCloudImages/29.png)
...
>⭐**为什么有的需要算分，有的不需要算分？**
>
>* 比如说搜索“JAVA基础培训”，只要跟JAVA基础培训有关的都可以，就算不叫“JAVA基础培训”，叫“JAVA入门”也可以搜索出来，这时候可能匹配度不一样，有的高有的低，这时候应该去算分。
>  * 但搜索完了之后想对搜索结果进行一个过滤，比如说我想搜索JAVA课程价格500到1000内的，这是一个价格的过滤，做价格范围过滤的时候要么就是符合条件的，要么就是不符合条件的，**不会存在一个匹配度的问题，这时候打分就没有意义**。
>    * 又比如说要对品牌进行过滤，只要“黑马”的课程，这时候搜索出来的就没有其他品牌的课程，这也**不存在匹配度的问题，就不应该打分**。
>
>⭐**总结：**
>
>* 通常情况下，用户**输入关键词**进行搜索，我们应该给他**算分**；但用来做**过滤筛选**的，通常情况下都**不应该算分**。这就是要做区分的原因。
>
>⭐**都参与算分的话有什么影响吗？既然不影响匹配度的话**
>
>* 因为算分是有一个函数进行算分，要给每一个文档算的话，如果要进行算分的字段和条件越多，将来计算得分时要消耗的性能就越多，效率也会变低所以能不算分就不算分
...
>**上述代码逻辑：**
>
>* 实际检索逻辑等价于：价格 ≤ 1000 且 name 包含 "手机"，品牌是 vivo/小米 的排前面。 must_not 中 gte: 2500 的条件被 filter 的 lte: 1000 完全覆盖，属于冗余条件。
>
>⭐**那这里的是打分还是不打分，还是怎么打分？**
>
>* 在 Elasticsearch 的 bool 查询中，各子句的打分规则如下：
>  参与打分（影响得分）：
>
>* | 子句     | 作用                             | 本查询中的表现                            |
>  | -------- | -------------------------------- | ----------------------------------------- |
...
>  | 子句       | 作用               | 本查询中的表现                       |
>  | ---------- | ------------------ | ------------------------------------ |
>  | `filter`   | 必须匹配，不打分   | `price <= 1000` 是硬过滤，结果可缓存 |
>  | `must_not` | 必须不匹配，不打分 | `price >= 2500` 仅排除文档           |
>
>  总分 = must 基础分 + should 加分
>  **must 提供的全文匹配分是基底，should 命中的品牌匹配分是增量。因此品牌是 vivo 或小米的文档会排在前面，但所有 price ≤ 1000 且 name 匹配 "手机" 的文档都会返回。**
>  must_not 中 gte: 2500 属于冗余条件——filter 已经把价格卡在 ≤1000，不可能有文档同时 ≥2500 了。

出于性能考虑，**与搜索关键字无关的查询尽量采用must_not或filter逻辑运算，避免参与相关性算分**。

例如黑马商城的搜索页面：

![](../SpringCloudImages/f09-011.png)
...
elasticsearch⭐**默认是根据相关度算分（`_score`）来排序**，但是也**支持自定义方式对搜索结果排序**。不过分词字段无法排序，⭐**能参与排序字段类型有：`keyword`类型、数值类型、地理坐标类型、日期类型等。**
...
> **注意：**
>
> * 可以看到这个sort字段是个数组，意味着可以指定**多个字段来进行排序**
>   * 多个的情况下就是：先按照第一个字段来排序，第一个字段一样的情况下就按照第二个来排序，以此类推。
...
> **举个通俗例子：**
>
> * 学校里有10个班级，想找出年级前10名，难道说每个班的第一名拿过来就是前十名了吗？显然不是，那是要把所有学生合在一起去找前10名吗？其实也不用。
>   * ⭐**只需要把每个班级的前10名找出来放一起比较即可，因为就算极端一点，有一个班的学习情况很好，领先所有人，最终年级前10就是这个班的前10，所以我们只要找每个班的前10就能找出年级前10。**

如图：

![](../SpringCloudImages/f09-012.png)
...
> * 一句话版：
>
>   * `search_after` 就像拿着“上一页最后一条记录的位置书签”，下一次从这个书签后面继续读，而不是每次都从第一页重新数到目标页。
>
> * 可以把 `search_after` 理解成：**不要告诉 ES“跳过前 10000 条”，而是告诉 ES“从上一页最后一条后面继续查”。**
>
>   普通分页是这样的：
>
>   ```
...
>   ```
>
>   意思是：先找到前 `10010` 条，再丢掉前 `10000` 条，只返回 10 条。页数越深，浪费越大，所以叫深度分页问题。
>
>   `search_after` 的思路是这样的：
>
>   第 1 页：
>
>   ```
...
>       { "id": "asc" }
>     ],
>     "search_after": [899, "10086"]
>   }
>   ```
>
>   意思是：**按照同样的排序规则，从 `price=899, id=10086` 这条记录之后继续取 10 条。**
>
> **⭐search_after字段是一个数组，然后这个数组里面固定两个元素吗？**
>
> * **关键规则：**
>
>   > `search_after` 的值来自上一页最后一条数据返回结果里的 `sort` 数组，原样传给下一次查询。
>
> * 如果你按两个字段排序：
>
>   ```
...
> ⭐**注意**：
>
> - 搜索必须有查询条件，而且是**全文检索类型**的查询条件，例如`match`
> - 参与高亮的字段必须是**`text`类型的字段**
> - 默认情况下参与**高亮的字段要与搜索字段一致**，除非添加：`required_field_match=false`



示例：

![](../SpringCloudImages/f09-014.png)

> 注意：
>
> * **是 **对 `fields` 指定字段里“命中的搜索关键字/词项”加高亮**，不是把整个字段都加粗。**
...
![](../SpringCloudImages/f09-017.png)

另一个是`QueryBuilders`，其中包含了我们学习过的各种**叶子查询**、**复合查询**等：

![](../SpringCloudImages/f09-018.png)
...
![](../SpringCloudImages/f09-026.png)

另外，我们还可以让聚合按照每个品牌的价格平均值排序：

![](../SpringCloudImages/f09-027.png)
...
![](../SpringCloudImages/f09-028.png)

聚合结果与搜索文档同一级别，因此需要单独获取和解析。具体解析语法如下：

![](../SpringCloudImages/f09-029.png)
...
![](../SpringCloudImages/f09-031.png)

### 4.2.过滤条件聚合

搜索页面的过滤项目前是写死的：

![](../SpringCloudImages/f09-032.png)
...
![](../SpringCloudImages/f09-033.png)

由于采用的是POST请求，所以参数在请求体中：

![](../SpringCloudImages/f09-034.png)
