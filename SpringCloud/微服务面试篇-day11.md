# 微服务面试篇（day11）：分布式事务 · 注册中心 · 远程调用 · 服务保护

> 本文档由飞书知识库《微服务框架 · day11-微服务面试篇》整理导出，保留原文结构与图片。
...
![](../SpringCloudImages/f11-002.png)

这种模式在大多数情况下（99%）并不会有什么问题，不过在极端情况下，特别是多线程并发访问AT模式的分布式事务时，有可能出现脏写问题，如图：

![](../SpringCloudImages/f11-003.png)

解决思路就是引入了全局锁的概念。在释放DB锁之前，先拿到全局锁。避免同一时刻有另外一个事务来操作当前数据。

![](../SpringCloudImages/f11-004.png)
...
![](../SpringCloudImages/f11-012.png)

默认所有的服务和配置都属于这个`namespace`，当然我们也可以自己创建新的`namespace`：

![](../SpringCloudImages/f11-013.png)

然后填写表单：

![](../SpringCloudImages/f11-014.png)

添加完成后，可以在页面看到我们新建的`namespace`，并且Nacos为我们自动生成了一个命名空间id：

![](../SpringCloudImages/f11-015.png)

我们切换到配置列表页，你会发现`dev`这个命名空间下没有任何配置：

![](../SpringCloudImages/f11-016.png)

因为之前我们添加的所有配置都在`public`下：

![](../SpringCloudImages/f11-017.png)
...
![](../SpringCloudImages/f11-018.png)

而其它服务则出现在`public`下：

![](../SpringCloudImages/f11-019.png)

此时访问`http://localhost:8082/doc.html`，基于`swagger`做测试：

![](../SpringCloudImages/f11-020.png)

会发现查询结果中缺少商品的最新价格信息。

我们查看服务运行日志：

![](../SpringCloudImages/f11-021.png)
...
![](../SpringCloudImages/f11-023.png)

查看nacos控制台，会发现默认情况下所有服务的集群都是default：

![](../SpringCloudImages/f11-024.png)
...
![](../SpringCloudImages/f11-025.png)

再次查看nacos：

![](../SpringCloudImages/f11-026.png)
...
![](../SpringCloudImages/f11-027.png)

我们可以用idea打开查看一下：

![](../SpringCloudImages/f11-028.png)
...
![](../SpringCloudImages/f11-042.png)

默认的实现是`RoundRobinLoadBalancer`，即**轮询**负载均衡器。负载均衡器的核心逻辑如下：

![](../SpringCloudImages/f11-043.png)
...
![](../SpringCloudImages/f11-050.png)

DEBUG重启后测试，会发现负载均衡器的类型确实切换成功：

![](../SpringCloudImages/f11-051.png)
...
![](../SpringCloudImages/f11-055.png)

可以看到原本是3个实例，经过筛选后还剩下2个实例。

查看Debug控制台：

![](../SpringCloudImages/f11-056.png)
...
![](../SpringCloudImages/f11-057.png)

那么问题来了， 这个权重是怎么配的呢？

我们打开nacos控制台，进入`item-service`的服务详情页，可以看到每个实例后面都有一个**编辑**按钮：

![](../SpringCloudImages/f11-058.png)

点击，可以看到一个编辑表单：

![](../SpringCloudImages/f11-059.png)

我们将这里的权重修改为5：

![](../SpringCloudImages/f11-060.png)
...
![](../SpringCloudImages/f11-061.jpg)

两者的优缺点如下：

![](../SpringCloudImages/f11-062.png)
