...
`Consistency`（一致性）：用户访问分布 2式系统中的任意节点，得到的数据必须一致。

比如现在包含两个节点，其中的初始数据是一致的：

![](../SpringCloudImages/43.png)

当我们修改其中一个节点的数据时，两者的数据产生了差异：

![](../SpringCloudImages/44.png)

要想保住一致性，就必须实现node01 到 node02的数据 同步：

![](../SpringCloudImages/45.png)
...
![](../SpringCloudImages/47.png)

⭐️⭐️**由于网络故障，当我们把数据写入node01时，可以与node02完成数据同步，但是无法同步给node03。现在有两种选择：**

- 允许用户任意读写，保证可用性。但由于node03无法完成同步，就会出现数据不一致的情况。满足`AP`
- 不允许用户写，可以读，直到网络恢复，分区消失。这样就确保了一致性，但牺牲了可用性。满足`CP`



⭐️可见，在分布式系统中，`A`和`C`之间只能满足一个。
...
### 1.3.AT模式的脏写问题(AT使用频率最高)

我们先回顾一下AT模式的流程，AT模式也分为两个阶段：

> AT模式的原理是需要先生成快照的，生成快照就是资源管理器去拦截这个sql的执行，生成一个快照，`生成快照的原理就是用这个update语句中的where条件来进行一个查询，把查询到的数据保存一份，这就是快照了，将来进行数据恢复的时候就可以用这个数据来恢复。`

> ⭐️⭐️⭐️⭐️⭐️
>
> ![](../SpringCloudImages/48.png)
>
> ![](../SpringCloudImages/49.png)
...
解决思路就是引入了`全局锁`的概念。在`释放DB锁之前，先拿到全局锁`。避免同一时刻有另外一个事务来操作当前数据。

![](../SpringCloudImages/f11-004.png)

> **⭐️⭐️⭐️⭐️⭐️全局锁机制实现数据隔离**
>
> * 当事务2获取全局锁的时候，这个TC记录表中已经有人获取了，所以不能再获取了
>   * 因为同一个table，同一个id，这两个将来可以做联合主键，视作唯一，这里已经有了，所以插入语不进来了，这个锁就无法获取了
> * 事务2无法获取全局锁的话，就会卡在那里重试，等事务1释放全局锁，但事务1要等二阶段结束才释放全局锁。
> * 事务1现在在干嘛，比如说事务1现在要回滚了，但要回滚的话就要拿到DB锁，也就是数据库锁，但DB锁在事务2那里。但要事务2释放DB锁就要让事务2先拿到全局锁。但事务1要拿到DB锁才能回滚。
> * 所以事务1等事务2释放DB锁，事务2等事务1释放全局锁。这就产生了死锁
> * 但在seata内部重试是有一个限制的，重试最多30次，但等待DB锁的过程是比较长的，所以事务2会先等不及，就会回滚业务，释放DB锁，所以并没有成功把money设置为80
> * 事务1拿到DB锁后就可以恢复了，在这个过程中没人修改过，这样问题就解决了

> **注意：AT和XA还是有区别的**
>
> * AT这里的TC锁，XA是全程数据库锁
> * 数据库的锁和TC锁有一个力度上的区别
> * 数据库上的锁如果不释放的话，任何人都无法来访问这条数据，不管是增删改查都不行
> * TC锁只是记录操作这个表这行数据的全局事务，由senta来管理的，如果你这个事务不是由senta来管理的，你去操作是没有影响的，比如说，我现在是在修改用户余额，但你现在有个其他业务也要来修改同样是id为1的数据，但修改是其他字段，这是可以的。
> * 所以AT模式尽管加了全局锁，性能还是比XA好。
>
> 不同事务来操作同一个字段是什么意思
...
> **有个极端情况：：**
>
> * 就是我现在在修改money的过程中，现在有个其他事务，seata没有管理的事务，他也来修改money，但它又不需要获取全局锁，这时候就会出问题。
>
> 
>
> **但这种情况其实出现率非常低**
>
> * 全局事务的执行大多数情况二阶段都是成功的，不是回滚而是提交。
> * 分布式事务本来业务耗时就长，因为链路长，因为执行效率和并发本来就比较低，那比较低的情况下刚好两个事务碰在一起的几率就比较低，并且在做业务的时候本身就要尽可能避免多个不同事务去操作同一个字段，如果真的出现了，那就想办法把另一个事务也交给senta去管理，这样就可以用全局锁去隔离了。
>
> 
>
> **解决办法：**
>
> ![](../SpringCloudImages/50.png)
>
> 
>
> **注意：**
>
> * 其实在senta内部，保存快照保存了两个快照，一个是更新前的快照，也就是100，一个是更新后的快照，也就是80
> * 但我们做恢复的时候，它不仅要拿到更新前的快照，还要拿到更新后的快照。更新前的快照目的是做恢复，更新后的快照目的是更当前数据做对比，如果一样的话就是在一二阶段之间没有人做动作；如果不一样的话就是有人做了动作，就会无法恢复。



>**什么是”不同事务“？**
>
>* 每一次独立的业务请求，通常都会创建自己的事务。
>* 在 Spring 中，常用 `@Transactional` 创建一个本地数据库事务：
>
>
>
>**什么是交给 Seata 管理？**
>
>* 普通 `@Transactional` 只能管理**当前服务、当前数据库连接**中的操作。例如订单服务写订单库，库存服务写库存库，两个服务各自的本地事务无法自动一起回滚。
>* Seata 用**全局事务**把多个本地事务协调起来
>
>
>
>**只要是事务去操控数据库的时候都会有数据库锁吗？**
>
>* 写操作通常会有锁
>  * 事务执行 `INSERT`、`UPDATE`、`DELETE` 时，数据库通常会对受影响的数据加锁，以保证并发修改不会把数据写乱。
>* 普通查询不一定加锁
>
>
>
>**数据库锁是数据库自动加上去的吗？**
>
>* 大多数情况下，是数据库根据你执行的 SQL 自动加的，不需要手动写“加锁”代码。
>
>* 例如：执行 `UPDATE` 时，MySQL InnoDB 会自动对 `account(id=1)` 加适当的写锁。该事务提交或回滚时，锁通常自动释放。
>
>  ~~~sql
>  BEGIN;
>  
...
> 1.假如有另外一个事务也来操作这个，也要冻结金额，那也没问题
>
> * 你要冻结的金额是你那部分，和我没关系![](../SpringCloudImages/51.png)
>
> 2.TCC模式靠的是资源预留，大家互不影响，既然互不影响就无需加锁就可以实现隔离效果
...
![](../SpringCloudImages/52.png)

> RM就是每一个分支事务
>
> **在 Seata 的角色划分中：**
>
> - **TM（Transaction Manager）**：事务发起者。负责开始、提交、回滚一个全局事务。
> - **TC（Transaction Coordinator）**：事务协调器，通常就是 Seata Server。负责记录全局事务、分支事务状态，并在最后协调提交或回滚。
> - **RM（Resource Manager）**：资源管理器。负责管理某个具体的事务资源，并向 TC 注册、汇报该资源对应的分支事务状态。
>
> **三者区别**
>
> | 角色 | 大白话职责                                   | 是否直接处理业务数据                     | 常见对应物                     |
> | ---- | -------------------------------------------- | ---------------------------------------- | ------------------------------ |
> | TM   | 事务发起人，决定“办不办这件事”               | 会调用业务服务，但不负责各资源的具体处理 | 订单服务的下单方法             |
> | TC   | 总调度和记账中心，决定“全体确认还是全体撤销” | 不直接改库存、余额等业务数据             | Seata Server                   |
> | RM   | 资源的实际负责人，负责“预留、确认、释放”     | 是                                       | 库存服务、账户服务、优惠券服务 |

> **我调用一个接口就可能出现多个RM吗？**
>
> 对，一个接口调用对应的一次业务流程，**完全可能出现多个 RM**。
> 例如用户调用下单接口：
> ```http
> POST /orders
> ```
> 订单服务处理这个接口时，发起一个 Seata 全局事务：
> ```text
> 创建订单（一个全局事务）
> ```
> 随后它调用多个服务：
...
> | 服务 / 资源          | 在这笔事务中的角色     |
> | -------------------- | ---------------------- |
> | 订单服务及订单库     | 一个 RM / 一个分支事务 |
> | 库存服务及库存库     | 一个 RM / 一个分支事务 |
> | 账户服务及账户库     | 一个 RM / 一个分支事务 |
> | 优惠券服务及优惠券库 | 一个 RM / 一个分支事务 |
> | 所以可以理解为：     |                        |
> ```text
> 一次接口请求
>   -> 一次全局事务
...
> ```
> 不过有两个细节：
> - **接口不一定会开启全局事务**。只有入口代码开启了 `@GlobalTransactional`，或从上游收到并传播了 Seata 的 `XID`，才会参与 Seata 全局事务。
> - **多个 RM 不等于必须调用多个微服务**。即使在一个单体应用中，同时操作订单库、库存库、账户库等不同资源，也可能有多个 RM 或多个分支事务。反过来，一个服务若只操作一个资源，通常只注册一个分支事务。
> 最简单的判断方式是：这笔业务一次要协调几个相对独立的资源，就可能有几个 RM 参与。

> 标记了 @GlobalTransactional注解的是TC还是TM？
>
> 标记了 `@GlobalTransactional` 的方法所属服务，扮演的是 **TM**
>
> 例如：
>
> java
...
> 这里：
>
> - `OrderService#createOrder()`：**TM**，它发起全局事务。
> - `inventoryService`：一个 **RM**，管理库存资源。
> - `accountService`：一个 **RM**，管理账户余额资源。
> - Seata Server：**TC**，负责协调全局事务。

如果阻塞时间太长，可能导致全局事务超时而触发二阶段的`cancel`操作。两个分支事务都会执行cancel操作：![](../SpringCloudImages/53.png)
...
> **discovery为什么要写namespace，意思是把当前服务注册到这个namepsace里去吗？**
>
> * 是的。`discovery.namespace` 用于指定**当前服务注册到哪个 Nacos 命名空间**。
...
综上，你会发现`Eureka是尽量不剔除服务，避免“误杀”，宁可放过一千，也不错杀一个。这就导致当服务真的出现故障时，迟迟不会被剔除，给服务的调用者带来困扰`。
...
- Eureka和Nacos都支持集群，而且**默认都是`AP`模式**
...
⭐默认的实现是`RoundRobinLoadBalancer`，即`轮询负载均衡器`。负载均衡器的核心逻辑如下：

> **轮询算法原理**
>
> * 就是一个`不断自增的数字对实例集合大小求余`，就可以实现轮询
...
- `RoundRobinLoadBalancer`：基于**轮询算法**实现了`ReactiveLoadBalancer`
- `RandomLoadBalancer`：基于**随机算法**实现了`ReactiveLoadBalancer`，

这样一来，整体思路就非常清楚了，流程图如下：

![](../SpringCloudImages/55.png)
...
>
> * **信号量隔离**
>   * 信号量隔离限制的是并发数，QPS 限制的是单位时间内的请求数。
>     * 例如信号量大小为 10：
>
>       * 同时最多允许 10 个请求执行
>       * 第 11 个请求可能被拒绝或降级
>       * 只要前面的请求完成，后续请求就可以继续进入
>       * 一秒内实际处理多少请求，取决于每个请求的耗时
> * **线程池隔离**
>   * 例如线程池大小为 10，不代表只能处理 10 QPS：
>     * 请求耗时 100ms，理论上可能接近 100 QPS
>     * 请求耗时 1s，理论上可能接近 10 QPS
>     * 如果请求持续阻塞，线程池很快会耗尽，后续请求进入队列或被拒绝
>
> | 方式       | 限制对象         | 是否切换线程 | 适合场景               |
> | ---------- | ---------------- | ------------ | ---------------------- |
> | 线程池隔离 | 线程数、队列容量 | 是           | 网络、数据库等阻塞调用 |
> | 信号量隔离 | 同时执行的请求数 | 否           | 快速的本地操作         |
> | QPS 限流   | 单位时间内请求数 | 通常否       | 控制流量速率           |
...
>
> * `Hystrix 默认是基于线程池实现的线程隔离`，每一个被隔离的业务都要创建一个独立的线程池，线程过多会带来额外的 CPU 开销，性能一般，但是隔离性更强。
>
...
>⭐⭐⭐⭐⭐`滑动窗口计数器算法`会将一个窗口划分为 n 个更小的区间，例如
>
>- 窗口时间跨度 Interval 为 1 秒；区间数量 n = 2 ，则每个小区间时间跨度为 500ms，每个区间都有计数器
>- 限流阈值依然为 3，时间窗口（1 秒）内请求超过阈值时，超出的请求被限流
...
> 应该是当请求来的那一刻，临时的去看一下这一刻往前推一秒，总共有多少个请求，是否达到阈值，达到了就拒绝，没达到就放行，这样的话才能更加精准的知道这一秒内的请求量，才能准确实现限流。但这样实现起来太复杂，不现实。
...
- 按照规则，currentTime-Interval值为300ms，300ms`之后`的第一个时区是500~1000，因此窗口范围包含两个时区：500~1000、1000~1500，也就是粉红色方框部分
...
> ⭐️⭐️比如说在第1s的时候很闲，没有请求来，这时候生产了10个令牌，首先假设令牌容量是10。那第1s的时候生产了10个令牌存起来，然后第2s一瞬间来了20个请求，这20个请求首先一瞬间把桶里的10个令牌消耗掉了，但由于到了第2s，所以会再生成10个令牌，于是这10个令牌也被消耗掉了，最终20个请求都被放行了。这一下子就处理了20个请求，是处理上限的一倍，这时候就触发了服务上限，可能把服务压垮。

因此，在使用令牌桶算法时，尽量不要将令牌上限设定到服务能承受的QPS上限。而是预留一定的波动空间，这样我们才能应对突发流量。

* 通常情况下，使用令牌组的时候，比如说服务器的承受上限是每秒钟是10， 那么生产令牌的速度就不能设置为10，可能设置为5，桶容量也可能设置为5，那么5+5就等于10，这时候正常情况就是5，极限情况下是10，这才不超过上限。但这样设置也有问题，就是明明每秒钟上限是10，结果90%情况每秒钟都只能处理5，这时候服务就显得有点空闲。

> 令牌桶限流适合于有非常多限流规则的场景，比如说在sentinal就把它用于热点参数限流，什么叫热点参数限流？
>
> * 意思就是不是针对接口的限流，是针对某个接口内方法内的参数，不同参数设置不同限流。
>   * 举个例子：有个根据id查商品的接口,有几个商品在参与秒杀，给这几个参与秒杀的商品单独设置限流规则，每个参数设置不同限流规则，其他商品再设置别的限流规则。
>     * 这时候就是每个参数都要用自己的令牌桶，这时候用令牌桶就成本很低，用滑动窗口就成本很高
>     * 由于是真单个参数做限流，不是针对整个接口，所以没有达到整个接口的上限是可以接受的，用令牌桶相对比较合适。



>⭐️**Sentinel的限流与Gateway的限流有什么差别？**
>
>- **问题说明：** 考察对限流算法的掌握情况
>- **难易程度：** 难
>- **参考话术：**
...
> ⭐️**Sentinel内部基于漏桶算法实现了排队等待效果，桶的容量取决限流的QPS阈值以及允许等待的最大超时时间：**
>
...
>
> ![](../SpringCloudImages/60.png)
>
> ![](../SpringCloudImages/61.png)
>
> 
