...
- 函数调用遵循`栈`结构，`最后被调用的函数最先返回（LIFO，Last In First Out，后进先出）`。
...
> **全局变量：** 在函数之外定义的变量，称之为`全局变量`，在整个文件中（包括函数内）都可以使用（通常定义在文件的顶部）。

> **局部变量：** 在函数内部定义的变量，称之为`局部变量`，只能在该函数内部使用，外部无法访问（函数执行完毕后，会自动销毁其内部的局部变量）。
...
> **注意：** 在基于 `global` 声明全局变量时，要`先声明，再使用`。
...
> ⭐注意：`默认参数必须放在没有默认值的参数列表的后面`，`一个函数`在定义时是可以设置`多个默认参数`的。
...
result = calc_sum(10, 20, 30, 40)
print("总和：", result)              #100
```

> ⭐**注意：** 传递的所有匹配的位置参数都会被 `args` 变量收集，这些参数会`合并封装为一个元组`，`args` 是元组类型（注意并不会封装关键字参数）。
...
def show_info(**kwargs):
    print(kwargs)           #{'name': '张三', 'age': 18, 'city': '北京'}
...
> **⭐注意：** 参数是以`“键=值”`形式传递的关键字参数，这些“键=值”参数都会被 `kwargs` 接受，并合并`封装为一个字典类型`。
...
> **注意**：`Python 是动态类型语言`，添加的类型注解`只是提示，并不是强制约束！`

### 函数类型注解

为函数添加类型注解，其实主要就是为`函数的参数和返回值添加类型注解`，具体语法如下：
